...
class Meaning < Word
  ...
  def meaning
      ...
      meaning = parsed_data[0]["meanings"][0]["definitions"][0]["definition"]
      meaning
  end
end
...
class Gameplay < Meaning
  ...
  def get_char
    ...
    char = gets.chomp
    unless (char.match(/[a-z]/) && char.length == 1)
  ...
  def welcome
    print "Welcome to a variation of Hangman\nNote: 20-30% of time, it fails to fetch the meaning, so rerun in that case\n "
  end
  ...
  def casket
    @word_casket = @word.split('').map{|chars| '_'}
    display_casket
  end
